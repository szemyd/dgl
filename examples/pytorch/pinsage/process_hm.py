...
if __name__ == '__main__':
    ...
    train_indices, val_indices, test_indices = train_test_split_by_time(transactions, 't_dat', 'customer_id')

    print("| Building Train Graph...")
    # Build the graph with training interactions only.
    train_g = build_train_graph(g, train_indices, 'customer', 'article', 'buys', 'rev_buys')
    assert train_g.out_degrees(etype='buys').min() > 0

    # Build the user-item sparse matrix for validation and test set.
    val_matrix, test_matrix = build_val_test_matrix(g, val_indices, test_indices, 'customer', 'article', 'buys')
